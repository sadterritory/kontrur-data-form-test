    ...
    <form id="userDataForm" class="userDataForm">
        ...
        <div class="wrapper">
            <div class="dataInputsWrapper">
                <div><input type="text" id="nameInput" placeholder="Введите имя" class="userDataInp" name="user_name"></div>
                <div><input type="tel" id="phoneInput" placeholder="Введите номер телефона" class="userDataInp" name="phone_number"></div>
                <div><input type="email" id="emailInput" placeholder="Введите адрес электронной почты"
                            class="userDataInp" name="email"></div>
            </div>
            <div class="sendButtonWrapper">
                <button id="submitButton" class="submitBtn">Отправить</button>
            </div>
        </div>
    </form>
</div>
    ...
    $(document).ready(function() {
        $('#userDataForm').on('submit', function (event) {
            ...
            $.ajax({
                ...
                error: function(xhr) {
                    alert('Произошла ошибка: ' + xhr.responseJSON?.message || 'Unknown error');
                }
            });
        });
    });
